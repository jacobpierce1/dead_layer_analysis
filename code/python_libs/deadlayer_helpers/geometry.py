...
def _populate_all_coords( all_coords, source_data ):
    ...
    for source in sources_index:
        ...
        x, y = [ error.esum( error.emean( source_data.loc[ source, col ], _source_data_delta ),
        ...
        z = error.esum_n( [ error.emean( enclosure_data['bottom_offset'], _source_data_delta ),
                            error.emean( source_data.loc[ source, 'height' ],
                                         _source_data_delta ),
                            error.emean( source_data.loc[ source, 'wafer' ], _source_data_delta )
        ] )

        xyz = np.array( [ var['value'] for var in [x,y,z] ] ) 
        xyz_delta =  np.array( [ var['delta'] for var in [x,y,z] ] )
        m = error.measurement( xyz, xyz_delta )
        all_coords.loc[source] = m * _MM_PER_INCH
        ...
        # all_coords[values_index[0]].loc[source] = xyz * _CM_PER_INCH
        # all_coords[values_index[1]].loc[source] = xyz_delta* _CM_PER_INCH
...
def get_costheta( coords ):
    ...
    return error.measurement( value, delta )
