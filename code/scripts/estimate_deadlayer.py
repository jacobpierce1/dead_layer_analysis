    ...
    for i in np.arange(len(E_current)):
        E_current[i] = (
            energy_entering_detector( p[0], p[1], p[2],
                                      _stop_power_interp_funcs[-1],
                                      x[i] ) )
        
    # todo: account for distance traveled by the alpha
    #
    #
...
# for E0.
def _energy_entering_detector( det_dl_efficiency, a, b, stop_power_interp_func, channel ):
